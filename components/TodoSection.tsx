...
// the manager's verdict is the source of truth. No background sweep
// resurfaces a checked row. (Earlier behavior: a 12-hour auto-reopen for
// "Recovered" rows still in the missed list — removed in 2026 because it
// produced false reopens when Tekmetric appointment sync lagged, the
// customer rebooked at a different shop, or the appointment was entered
// manually outside the source the missed-list reads from. See the
// reopenStale* functions in lib/{callback,rebook,declinedJob}Store.ts.)
  ...
  return (
    <div className="card mb-8">
      <div className="flex items-start justify-between gap-3 flex-wrap mb-4">
        <div className="flex items-center gap-2.5">
          <span className="inline-flex items-center justify-center w-9 h-9 rounded-full" style={{ background: 'rgba(245,166,35,0.12)' }}>
            <ListChecks className="w-[18px] h-[18px]" style={{ color: '#F5A623' }} />
          </span>
          <div>
            <h2 className="section-h">To Do — Calls to Make Today</h2>
            <p className="section-sub mt-0.5">Pick a shop, then a queue.</p>
          </div>
        </div>

        {/* Presence chips — "who else is currently working the queue."
            Drops off within ~5 s of a user switching tabs (via the Page
            Visibility API + sendBeacon leave). */}
        {otherActiveUsers.length > 0 && (
          <div className="flex items-center gap-1.5 flex-wrap text-[11px] text-mango-muted shrink-0">
            <span className="font-semibold uppercase tracking-wide text-mango-faint">Active now:</span>
            {otherActiveUsers.map(u => {
              const sameShop = u.shopNum === shopNum;
              const shopMeta = SHOP_BY_NUM[u.shopNum as keyof typeof SHOP_BY_NUM];
              return (
                <span key={u.email}
                  className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full bg-white border border-mango-line"
                  title={`${u.email} · viewing ${shopMeta?.name ?? u.shopNum}`}>
                  <span className="inline-block w-1.5 h-1.5 rounded-full bg-emerald-500" />
                  <span className="font-medium text-mango-ink">{shortActor(u.email)}</span>
                  {!sameShop && shopMeta && (
                    <span className="text-mango-faint">· {shopMeta.name}</span>
                  )}
                </span>
              );
            })}
            ...
            <button
              ...
              className={`shrink-0 inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full border text-sm font-medium transition ${
                active
                  ...
                  : 'bg-white text-mango-ink border-mango-line hover:border-mango-muted'
              }`}
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 mb-4">
        <div className="rounded-lg border border-mango-red/25 bg-mango-red/8 p-3 flex items-baseline justify-between gap-3">
          <div className="text-[11px] uppercase tracking-wide text-mango-red/80 font-semibold">Total revenue at stake this shop</div>
          <div className="text-2xl font-bold tabular-nums text-mango-red shrink-0">{grandTotalDollars > 0 ? usdK(grandTotalDollars) : '—'}</div>
        </div>
        <div className="rounded-lg border border-emerald-500/30 bg-emerald-500/8 p-3 flex items-baseline justify-between gap-3"
          title="Open items checked off as recovered this week (across all three queues for this shop)">
          <div className="text-[11px] uppercase tracking-wide text-emerald-700/90 font-semibold">Customers recovered this week</div>
          <div className="text-2xl font-bold tabular-nums text-emerald-600 shrink-0">{recoveredThisWeek}</div>
        </div>
      </div>
...
// Pill-style segmented control. Active = filled orange + white text so the
// selection is unmistakable; inactive = white with a thin border. Mirrors
// the shop selector above so the page reads as two stacked filter strips.
function TabButton({ active, onClick, icon, label, count, subtotal, loading }: {
  ...
  return (
    <button
      onClick={onClick}
      aria-pressed={active}
      className={`shrink-0 inline-flex items-center gap-2 px-3.5 py-2 rounded-full border text-[13px] transition ${
        active
          ? 'bg-mango-orange text-white border-mango-orange shadow-sm'
          : 'bg-white text-mango-ink border-mango-line hover:border-mango-orange/40'
      }`}
    >
      {icon}
      <span className="font-semibold">{label}</span>
      <span className={`text-[11px] tabular-nums ${active ? 'text-white/85' : 'text-mango-muted'}`}>

...
import { SE_SHOP_CACHE_KEY, getSalesCallHandled, markSalesCallHandled } from '@/lib/salesEffectiveness';
...
export async function handle(req: NextRequest) {
  ...
  // POST to mark a call as handled (employee+ allowed)
  if (req.method === 'POST') {
    if (!role) return NextResponse.json({ error: 'unauthorized' }, { status: 401 });
    const body = await req.json().catch(() => null);
    const { shopNum, callId } = body ?? {};

...
export const maxDuration = 120; // allow the cold 8-shop compute room to finish
...
async function computeAndCache(view: string, cacheKey: string) {
  ...
  const weekStart = startOfWeek(nowMtn, { weekStartsOn: 1 });
    ...
    if (view === 'leaderboard') {
      ...
      // Fan out all shops in parallel — each is independent and has its own
      // try/catch so a single shop failure still returns zeros for that shop.
      const results = await Promise.all(SHOPS.map(async (shop) => {
        try {
          const [ros, appts] = await Promise.all([
            rosForShop(shop.tekmetricId, { startISO: wkStartISO, endISO: wkEndISO }),
            fetchAllAppointments({ shopId: shop.tekmetricId, startTimeFrom: apptFromISO, startTimeTo: apptToISO }),
          ]);
    ...
    if (view === 'heatmap_12w') {
      // Parallelize shops. Within each shop weeks stay sequential to avoid
      // saturating the Tekmetric appointment endpoint (12w × 8 shops = 96 calls).
      const out = await Promise.all(SHOPS.map(async (shop) => {
        const weeks: Array<{ weekStart: string; fbrPct: number }> = [];
        for (let i = 11; i >= 0; i--) {
          const ws = addDays(weekStart, -7 * i);
          const we = addDays(ws, 6);
          const wsISO = fromZonedTime(ws, CHAIN_TZ).toISOString();
          const weISO = fromZonedTime(we, CHAIN_TZ).toISOString();
          const [ros, appts] = await Promise.all([
            rosForShop(shop.tekmetricId, { startISO: wsISO, endISO: weISO }),
        ...
        return { shopNum: shop.num, shopName: shop.name, weeks };
      }));
